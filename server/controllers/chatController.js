...
// @desc    Get one chat by ID
// @route   GET /api/chats/:id
const getChatById = async (req, res) => {
  const chat = await Chat.findOne({ _id: req.params.id, user: req.user._id, isArchived: false });
...
// @desc    Get chat stats for dashboard
// @route   GET /api/chats/stats
const getStats = async (req, res) => {
  const chats = await Chat.find({ user: req.user._id, isArchived: false });

  const totalChats = chats.length;
  const totalDoubts = chats.reduce(
    (sum, chat) => sum + chat.messages.filter((m) => m.role === 'user').length,
    0
  );

  const subjectBreakdown = chats.reduce((acc, chat) => {
    const key = chat.subject || 'General';
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});
  ...
  res.json({
    success: true,
    stats: { totalChats, totalDoubts, subjectBreakdown },
  });
};
